...
public class MessageController {
    ...
    // Replace openConversationWithUser() to ensure contacts are loaded before opening
    public void openConversationWithUser(int userId) {
        try {
            User contact = userService.getById(userId);
            if (contact == null) return;

            // ✅ If contact not yet in list, add them so the sidebar shows them
            boolean alreadyInList = contactList.getItems().stream()
                    .anyMatch(u -> u.getId() == userId);
            if (!alreadyInList) {
                contactList.getItems().add(0, contact); // add to top
            }
