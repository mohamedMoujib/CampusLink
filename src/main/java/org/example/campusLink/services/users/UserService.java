...
public class UserService implements Iservice<User> {
    ...
    public List<User> recuperer() throws SQLException {
        ...
        try (
            ...
            while (rs.next()) {
                ...
                if (user != null) {  // skip rows with null/empty/unknown user_type
                    users.add(user);
                }
            }
        }
    ...
    private void modifierAdmin(Admin admin) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setString(4, admin.getPhone());
            ps.setString(5, admin.getGender());
            ps.setString(6, admin.getProfilePicture());
    ...
    private User createUserFromResultSet(ResultSet rs) throws SQLException {
        ...
        // Guard against null or empty user_type in the DB
        if (userType == null || userType.isBlank()) {
            System.err.println("⚠️ Skipping row with null/empty user_type, id=" + rs.getInt("id"));
            return null;
        }

        switch (userType) {
            case "ETUDIANT":    return createEtudiant(rs);
            case "PRESTATAIRE": return createPrestataire(rs);
            case "ADMIN":       return createAdmin(rs);
            default:
                System.err.println("⚠️ Unknown user_type: '" + userType + "', id=" + rs.getInt("id"));
                return null; // skip unknown types instead of crashing
        }
    }
    ...
    private void validateUser(User user) {
        ...
        if (user.getProfilePicture() != null
                ...
                && !UserValidator.isValidURL(user.getProfilePicture())) {
