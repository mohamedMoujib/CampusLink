...
public class PaymentController {
    ...
    private void handleSubmit() {
        ...
        try {
            ...
            String studentName = currentUser != null ? currentUser.getName() : "Étudiant";
            ...
            // ── Invoice for STUDENT (etudiant) ──────────────────────────────
            String studentDetails = "Vous avez réservé le service \""
                    + service + "\" le " + dateStr
                    + " au lieu : " + meetingAddress + ".";

            Invoices studentInvoice = new Invoices();
            studentInvoice.setPaymentId(paymentId);
            studentInvoice.setInvoiceDate(now);
            studentInvoice.setDetails(studentDetails);
            studentInvoice.setUserId(currentUser != null ? currentUser.getId() : null);
            invoiceService.ajouter(studentInvoice);

            // ── Invoice for PROVIDER (prestataire) ──────────────────────────
            String providerDetails = "Votre service \"" + service
                    + "\" a été réservé par " + studentName
                    + " le " + dateStr
                    + " au lieu : " + meetingAddress + ".";

            Invoices providerInvoice = new Invoices();
            providerInvoice.setPaymentId(paymentId);
            providerInvoice.setInvoiceDate(now);
            providerInvoice.setDetails(providerDetails);
            providerInvoice.setUserId(currentReservation != null
                    ? currentReservation.getProviderId() : null);
            invoiceService.ajouter(providerInvoice);
