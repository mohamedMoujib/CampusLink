...
public class Student_controller {
    ...
    /**
     * Resolves an image filename to an actual File on disk.
     * Checks the absolute public/uploads/ directory.
     */
    private File resolveImageFile(String imageFilename) {
        if (imageFilename == null || imageFilename.isBlank()) return null;

        File file = null;

        // Handle legacy paths like "uploads/xxx.png"
        if (imageFilename.startsWith("uploads/")) {
            String filename = imageFilename.substring("uploads/".length());
            file = new File(UPLOAD_BASE_DIR, filename);
            if (file.exists()) {
                System.out.println("✅ Service image loaded from new location: " + filename);
                return file;
            }
            // Fall back to legacy path
            file = new File(imageFilename);
            if (file.exists()) {
                System.out.println("✅ Service image loaded from legacy: " + imageFilename);
                return file;
            }
        } else {
            // Try absolute public/uploads/ location
            file = new File(UPLOAD_BASE_DIR, imageFilename);
            if (file.exists()) {
                System.out.println("✅ Service image loaded: " + imageFilename);
                return file;
            }

            // Try legacy services location as fallback
            file = new File("uploads/services", imageFilename);
            if (file.exists()) {
                System.out.println("✅ Service image loaded from legacy: " + imageFilename);
                return file;
            }
        ...
        System.err.println("❌ Service image not found: " + imageFilename);
        return null;
    }
    ...
    private VBox createServiceCard(Services service) {
        ...
        if (service.getImage() != null && !service.getImage().isEmpty()) {
            File imageFile = resolveImageFile(service.getImage());
