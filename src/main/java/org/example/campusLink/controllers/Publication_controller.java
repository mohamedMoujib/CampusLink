...
public class Publication_controller {
    ...
    public void setUser(User user) {
        if (user != null) {
            this.currentStudentId = user.getId();
        }
    }
     ...
     *
     *   1. Filename only  → "1234_photo.png"
     *      Resolved from the absolute public/uploads/ directory.
     *
     *   2. Relative path → "uploads/publications/1234_photo.png" or "uploads/services/1234_photo.png"
     *      (legacy format from old upload code)
     *
     *   3. Absolute file URI → "file:/C:/Users/ALI/OneDrive/Images/photo.jpg"
     *      Parsed with URI so Windows paths are handled correctly.
     *
     *   4. Absolute native path → "C:/Users/ALI/..."  (edge case)
     *      Wrapped directly in new File().
     *
     * Returns null if the string is blank or the file does not exist.
     */
    private static final String UPLOAD_BASE_DIR = "C:/Users/ALI/OneDrive/Bureau/cam/Esprit-PIDEV-3A13-2526-CampusLink/pi/Esprit-PIDEV-3A13-2526-CampusLink/Esprit-PIDEV-3A13-2526-CampusLink/public/uploads";
    ...
    private File resolveImageFile(String imageUrl) {
        ...
        try {
            ...
            if (imageUrl.startsWith("file:")) {
                // Case: absolute file URI — use URI parser (handles %20 spaces etc.)
                file = new File(new URI(imageUrl));
            } else if (imageUrl.contains(":\\") || imageUrl.startsWith("/")) {
                // Case: absolute native path (Windows C:\ or Unix /)
                file = new File(imageUrl);
            } else if (imageUrl.startsWith("uploads/")) {
                // Case: legacy relative path → "uploads/xxx.png"
                // Try the new absolute path first
                String filename = imageUrl.substring("uploads/".length());
                file = new File(UPLOAD_BASE_DIR, filename);
                if (file.exists()) {
                    System.out.println("✅ Publication image loaded from new location: " + filename);
                    return file;
                }
                // Fall back to legacy path
                file = new File(imageUrl);
            } else {
                // Case: filename only — resolve from absolute public/uploads/
                file = new File(UPLOAD_BASE_DIR, imageUrl);
            }
    ...
    private void goToCreatePublication() {
        ...
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/Views/Create_Publication.fxml"));
            Parent view = loader.load();

            CreatePublication_controller ctrl = loader.getController();
            ctrl.setCurrentStudentId(currentStudentId);
            ctrl.setMainLayoutController(mainLayoutController); // ✅ pass reference back

            mainLayoutController.loadContent(view); // ✅ swap content area
