...
public class StudentReviewController {
    ...
    @FXML private ComboBox<ReservationItem> reservationComboBox;
    ...
    // ================= INNER CLASS: ReservationItem =================

    public static class ReservationItem {
        private final int reservationId;
        private final int prestataireId;
        private final String serviceTitle;
        private final String prestataireName;
        private final String displayText;

        public ReservationItem(int reservationId, int prestataireId,
                               String serviceTitle, String prestataireName) {
            this.reservationId = reservationId;
            this.prestataireId = prestataireId;
            this.serviceTitle = serviceTitle;
            this.prestataireName = prestataireName;
            this.displayText = String.format("%s - avec %s", serviceTitle, prestataireName);
        }
    ...
    private void showAddForm() {
        ...
        reservationComboBox.setVisible(true);
        reservationComboBox.setManaged(true);
    }
    ...
    private void loadConfirmedReservations() {
        ...
        List<Map<String, Object>> reservations =
                reviewsService.getConfirmedReservationsForStudent(getStudentId());

        reservationComboBox.getItems().clear();

        if (reservations.isEmpty()) {
            reservationComboBox.setPromptText("Aucune réservation confirmée disponible");
            reservationComboBox.setDisable(true);
            return;
        }

        reservationComboBox.setDisable(false);
        reservationComboBox.setPromptText("Sélectionnez une réservation");

        for (Map<String, Object> res : reservations) {
            ReservationItem item = new ReservationItem(
                    (Integer) res.get("reservationId"),
                    (Integer) res.get("prestataireId"),
                    (String) res.get("serviceTitle"),
                    (String) res.get("prestataireName")
            );
            reservationComboBox.getItems().add(item);
        }
    }
    ...
    private void loadReviews() {
        ...
        List<Reviews> reviews = reviewsService.getReviewsByStudentWithDetails(getStudentId());
    ...
    private void startEditing(Reviews review) {
        ...
        reservationComboBox.setVisible(false);
        reservationComboBox.setManaged(false);
    }
    ...
    private void handleAddReview() {
        ...
        if (editingReviewId != null) {
            ...
            ReservationItem selectedReservation = reservationComboBox.getValue();

            if (selectedReservation == null) {
                showAlert("Réservation requise", "Veuillez sélectionner une réservation.");
                return;
            }

            Reviews review = new Reviews(
                    getStudentId(),
                    selectedReservation.getPrestataireId(),
            ...
            try {
                ...
                showAlert("Avis déjà existant", e.getMessage());
                return;
            }
        }
